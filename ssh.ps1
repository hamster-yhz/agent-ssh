...
$ConfigPath = if ($env:SSH_SPACE_CONFIG) {
...
$AskPassSourcePath = Join-Path $WorkspaceRoot 'app\helpers\SshSpace.AskPass.cs'
$AskPassExePath = Join-Path $WorkspaceRoot 'data\ssh-space-askpass.exe'
...
Usage:
  ...
  .\ssh.ps1 import <path...>          Import one or many packages
  .\ssh.ps1 reset                    Restore factory defaults
  .\ssh.ps1 <alias>                  Start an SSH session
  .\ssh.ps1 <alias> <command...>     Run a remote command

Examples:
  .\ssh.ps1 dev
  .\ssh.ps1 dev uptime
  .\ssh.ps1 dev "docker ps --format '{{.Names}}'"
'@ | Write-Host
}
...
function Assert-OpenSshAvailable {
    ...
    if (-not $status.Available) {
        throw 'OpenSSH Client is unavailable. Reinstall SSH Space or enable the Windows OpenSSH Client.'
    }
    return $status
}
...
function Invoke-SshConnection {
    ...
    $previousPassword = $env:SSH_SPACE_PASSWORD
    try {
        ...
        $env:SSH_SPACE_PASSWORD = $previousPassword
    }
}
...
function Reset-SshSpaceFactoryDefaults {
    param([Parameter(Mandatory)][string]$Confirmation)
    if ($Confirmation -cne 'RESET SSH SPACE') {
...
try {
    ...
    if ($Target -eq 'export') {
        if ($null -eq $RemoteCommand -or $RemoteCommand.Length -lt 1) {
            throw 'Usage: .\ssh.ps1 export <alias|all> [output-directory]'
        }
    ...
    if ($Target -eq 'export-many') {
        if ($null -eq $RemoteCommand -or $RemoteCommand.Length -lt 1) {
            throw 'Usage: .\ssh.ps1 export-many <alias1> <alias2> [...]'
        }
    ...
    if ($Target -eq 'import') {
        if ($null -eq $RemoteCommand -or $RemoteCommand.Length -lt 1) {
            throw 'Usage: .\ssh.ps1 import <server.json|package-directory> [more paths...]'
        }
