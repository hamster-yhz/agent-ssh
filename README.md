...
SSH Space 会在启动后静默检查 GitHub 的最新稳定 Release，也可以点击右上角的下载图标手动检查。自动检查失败不会影响服务器管理和 SSH 连接。
...
原位升级只替换随安装包发布的程序文件，不会覆盖 `config\servers.local.json`、`keys\`、`exports\` 或 `backups\` 中的本地用户数据。
